...
describe('testing App Status Endpoints', () => {
  describe('gET /status', () => {
    it('returns the status of redis and mongo connection', async () => {
      const resp = await request(app).get('/status').send();
      const bd = JSON.parse(resp.text);

      expect(bd).to.eql({ redis: true, db: true });
  ...
  describe('gET /stats', () => {
    ...
    it('returns number of users and files in db 0 for this one', async () => {
      const resp = await request(app).get('/stats').send();
      const bd = JSON.parse(resp.text);

      expect(bd).to.eql({ users: 0, files: 0 });
      expect(resp.statusCode).to.equal(200);
    });
    ...
    it('returns number of users and files in db 1 and 2 for this one', async () => {
      ...
      expect(bd).to.eql({ users: 1, files: 2 });
      expect(resp.statusCode).to.equal(200);
    });
  });
});
